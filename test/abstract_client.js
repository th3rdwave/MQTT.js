...
const serverBuilder =
  require('./server_helpers_for_client_tests').serverBuilder;
...
module.exports = function (server, config) {
  ...
  describe('publishing', function () {
    ...
    qosTests.forEach(function (QoS) {
      it(
        'should publish 10 QoS ' +
          QoS +
          'and receive them only when `handleMessage` finishes',
        function (done) {
      ...
      client.once('connect', function () {
        ...
        store.put(
          ...
          function () {
            ...
            client._sendPacket = sinon.spy();
            client._handlePubrel(
              { cmd: 'pubrel', messageId: messageId },
              function (err) {
                assert.exists(err);
                assert.strictEqual(client._sendPacket.callCount, 0);
                client.end(true, done);
    ...
    callbackStorePutByQoSParameters.forEach(function (test) {
      if (test.args[0] === 0) {
        // QoS 0
        it(
          'should not call cbStorePut when publishing message with QoS `' +
            test.args[0] +
            '` and clean `' +
            test.args[1] +
            '`',
        ...
        // QoS 1 and 2
        it(
          'should call cbStorePut before publish completes when publishing message with QoS `' +
            test.args[0] +
            '` and clean `' +
            test.args[1] +
            '`',
  ...
  describe('qos handling', function () {
    ...
    function testMultiplePubrel(shouldSendPubcompFail, done) {
      ...
      client._sendPacket = function (packet, sendDone) {
        ...
        if (sendDone) {
          sendDone(
            shouldSendFail ? new Error('testing pubcomp failure') : undefined,
          );
        }
